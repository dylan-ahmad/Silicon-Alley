...
- **UI icon set (#55)** — per-concept icons live in `Assets/Mods/SiliconAlley/UI/Icons/`, named by the
  concept's `NameKey` minus `siliconalley:` (e.g. `feature_office_cloudsync.png`). `SiliconAlleyTheme.Load`
  loads the whole folder into a name→sprite map; **`SiliconAlleyTheme.IconFor(nameKey)`** resolves exact →
  `cat_<category>` placeholder → `null` (graceful — no broken sprite). Adding art is **drop-in** (no code).
  The full **game-icons.net** per-concept set (CC BY 3.0, recolored white-on-transparent) is **bundled**
  and attributed in `CREDITS.md` (root; `docs/` is gitignored). The `cat_*` placeholders (procedurally
  (re)generated by **Big Ambitions → Silicon Alley → Generate Placeholder Icons**) remain the per-category
  fallback. To swap an icon, drop a white-on-transparent PNG named for the concept (the mod tints it).
